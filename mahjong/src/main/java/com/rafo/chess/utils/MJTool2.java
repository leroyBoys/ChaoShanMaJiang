...
public class MJTool2 {
    ...
    public static void main(String[] args) {
      ...
      /*  hucardMap.add(new int[]{11,11,14,14,21,21,22,22,26,26,27,27,29,29});//七对
        ...
        Properties properties = PropertiesUtil.loadPropertyInstance("E:/workspace/ZiGongMahjongServer/mahjong/conf","config.properties");
        System.out.println(Json.toJson(properties));


        PropertiesUtil.getProperty("E:/workspace/ZiGongMahjongServer/mahjong/conf","config.properties","system_config_init");

        System.out.println(Json.toJson(new String[][]{new String[]{"a","b","c"},new String[]{"a","b","c"}}));
...
//            }
    ...
    private static boolean isQiDui(int[][] cards){
        int count;
    ...
    private static boolean isQiDuiLaiZi(int[][] cards,int laiZiCount){
        int count;
        for (int i = 1;i<cards.length;i++){
            if(cards[i] == null){
                continue;
    ...
    public static HuType daDuiZiCheck(int[][] cards){
        for (int i = 1;i<cards.length;i++){
            if(cards[i]==null){
                continue;
            }
    ...
    public static boolean isHu(int[][] cards){
        ...
        for(int i = 0;i<cards.length;i++){
            ...
            cardsCopy[i] = cards[i].clone();
        }

        HuData huData = new HuData();
        return huCheck(cardsCopy,huData);
    }
    ...
    private static boolean isHu(List<MJCard> mjCards, int extraCard,int laiZiCount){
    ...
    private static boolean isHu(List<MJCard> mjCards, int extraCard){
     ...
     * @return
     */
    private static boolean isHu(int[][] cards,int laiZiCount){
        if(cards == null){
            return false;
        }
