...
public class LiuJuPlugin implements IOptPlugin<DealerLiujuAction> {
	...
	private int getPro(MJPlayer mjPlayer){
		if(mjPlayer.isTing()){
			return 1;
		}else if(mjPlayer.getJiaozui() != null){
			return 2;
		}
	...
	public boolean analysis(DealerLiujuAction action) {
		List<MJCard> list = action.getRoomInstance().getEngine().getCardPool();
	...
	public void chaJiao(DealerLiujuAction action) {
		...
		for (IPlayer other : others) {
			...
			Set<Integer> mayBeHuCards = p.isTing()? p.getTingHuCards():null;
			Map<Integer,BaseHuRate> jiaoZui = ActionManager.jiaozuiCheck(p.getHandCards().getHandCards(),p,mayBeHuCards);
