...
public abstract class HmHuPlugin extends HuPlugin {
	...
	public void createCanExecuteAction(HuAction action) {
		...
		int nextFocusIndex = gameRoom.nextFocusIndex();
		if(nextFocusIndex < 0){
			action.getRoomInstance().setRoomStatus(GameRoom.RoomState.calculated.getValue());
			//设置未胡玩家的叫嘴状态
			checkJiaoZui(gameRoom);
			return;
		}
		ActionManager.moCheck(gameRoom.getPlayerArr()[nextFocusIndex],false,0);
	}
	...
	public boolean doPayDetail(PayDetail pd, GameRoom room, Calculator calculator) {
		...
		if(room.getPlayerById(pd.getToUid()).isHavHu()){
			...
			battleBalance.setHuTurnIdex(room.getLastWinner().getHuTurnIdex(pd.getToUid()));
		}
		return false;
	}
}
